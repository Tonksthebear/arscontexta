---
name: tutorial
description: Interactive walkthrough for new users. Learn by doing — each step creates real content in your vault. Three tracks (researcher, manager, personal) with a universal learning arc.
user-invocable: true
version: "1.0"
...
Display header, then use one concise question and wait for the answer:
...
Use one concise question and wait for the answer with track-adapted prompt:
...
No one concise question and wait for the answer. Run automated mini health check on tutorial {vocabulary.note_plural}:
...
Then use one concise question and wait for the answer:
