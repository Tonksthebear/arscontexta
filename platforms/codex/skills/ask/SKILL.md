...
This is the Codex-native version of the Ars Contexta skill. Invoke it by asking for the workflow in plain language; slash commands in examples are aliases from the Claude version, not requirements.

When instructions mention `$ARGUMENTS`, treat that as the user request target or the relevant file/note/topic from the current prompt. If the target is missing, ask one concise clarifying question and wait.
...
**Location:** `the Ars Contexta plugin root/methodology/` — filter by `kind: research`
...
**Location:** `the Ars Contexta plugin root/methodology/` — filter by `kind: guidance`
...
**Location:** `the Ars Contexta plugin root/methodology/` — filter by `kind: example`
...
**Search strategy:** Use `mcp__qmd__vector_search` across the `methodology` collection for semantic domain matching. To list all examples: `rg '^kind: example' the Ars Contexta plugin root/methodology/`.

### Reference Documents (structured derivation context)

**Location:** `the Ars Contexta plugin root/reference/`
...
Read `the Ars Contexta plugin root/reference/claim-map.md` first. This is the routing index — it shows which topic areas are relevant to the user's question and which claims to start with. Do NOT skip this step and search blindly.
...
- direct file reads/grep on `the Ars Contexta plugin root/methodology/` and `the Ars Contexta plugin root/reference/`
...
2. **Apply vocabulary** — use `the Ars Contexta plugin root/reference/vocabulary-transforms.md` to translate universal terms into their domain language. Answer about "reflections" not "claims" if they are running a therapy system
3. **Check constraints** — reference `the Ars Contexta plugin root/reference/interaction-constraints.md` to see if their configuration creates specific pressures relevant to the question
4. **Cite dimension-specific research** — use `the Ars Contexta plugin root/reference/dimension-claim-map.md` to ground answers in the specific claims that inform their configuration
...
Use `the Ars Contexta plugin root/reference/vocabulary-transforms.md` to translate universal terms into their domain language. This is not cosmetic — it is about making the answer *native* to their system.
...
Reference `the Ars Contexta plugin root/reference/interaction-constraints.md` to understand whether their configuration creates specific pressures relevant to the question. Some dimension combinations create tensions that affect the answer:
...
Use `the Ars Contexta plugin root/reference/dimension-claim-map.md` to ground answers in the specific claims that inform their configuration choices. This makes the answer traceable: "Your system does X because claim Y supports it for your configuration."
