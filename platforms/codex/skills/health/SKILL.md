...
3. **Three-space reference** — `the Ars Contexta plugin root/reference/three-spaces.md` for boundary rules (load only for full and three-space modes)
...
**If `validate-kernel.sh` exists** in `the Ars Contexta plugin root/reference/`, run it and include results.
...
**Before running this check:** Read `the Ars Contexta plugin root/reference/three-spaces.md` for the full boundary specification.
