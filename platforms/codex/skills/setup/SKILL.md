...
This is the Codex-native version of the Ars Contexta setup skill. Invoke it by asking in plain language, such as "set up my knowledge system" or "set up Ars Contexta in advanced mode".

When instructions mention `$ARGUMENTS`, treat that as the user request target or setup mode from the current prompt. If the target is missing, proceed with normal setup; ask one concise clarifying question only when required.
...
This source contains platform-specific branches for Claude Code, Codex, and minimal runtimes. In Codex, follow the Codex branches and translate any Claude slash-command examples into natural-language requests.
...
- `the Ars Contexta plugin root/reference/kernel.yaml` -- the 15 kernel primitives (with enforcement levels)
- `the Ars Contexta plugin root/reference/interaction-constraints.md` -- dimension coupling rules, hard/soft constraint checks
- `the Ars Contexta plugin root/reference/failure-modes.md` -- 10 failure modes with domain vulnerability matrix
- `the Ars Contexta plugin root/reference/vocabulary-transforms.md` -- domain-native vocabulary mappings (6 transformation levels)
- `the Ars Contexta plugin root/reference/personality-layer.md` -- personality derivation (4 dimensions, conflict resolution, artifact transformation)
- `the Ars Contexta plugin root/reference/three-spaces.md` -- three-space architecture (self/notes/ops separation rules)
- `the Ars Contexta plugin root/reference/use-case-presets.md` -- 3 presets with pre-validated configurations
- `the Ars Contexta plugin root/reference/conversation-patterns.md` -- 5 worked examples validating derivation heuristics

**Generation references (read during Phase 5):**
- `the Ars Contexta plugin root/generators/claude-md.md` -- CLAUDE.md generation template
- `the Ars Contexta plugin root/platforms/codex/generator.md` -- CODEX.md generation reference
- `the Ars Contexta plugin root/platforms/codex/skills.md` -- Codex skill translation rules
- `the Ars Contexta plugin root/platforms/codex/hooks/codex-hooks.json.template` -- Codex generated-vault hook template
- `the Ars Contexta plugin root/platforms/codex/hooks/codex-hooks.sh.template` -- Codex hook dispatcher template
- `the Ars Contexta plugin root/generators/features/*.md` -- composable feature blocks for context file composition
...
Once primary dimensions are set, cascade through interaction constraints. Read `the Ars Contexta plugin root/reference/interaction-constraints.md` for the full cascade rules.
...
Build the complete vocabulary mapping for all 6 transformation levels (see `the Ars Contexta plugin root/reference/vocabulary-transforms.md`):
...
Map personality signals to four dimensions (see `the Ars Contexta plugin root/reference/personality-layer.md`):
...
Run BEFORE proceeding to the proposal. Read `the Ars Contexta plugin root/reference/interaction-constraints.md`.
...
Read `the Ars Contexta plugin root/reference/failure-modes.md`. Check the derived configuration against the domain vulnerability matrix. Flag all HIGH-risk failure modes for this configuration. These get included in the generated context file's "Common Pitfalls" section.
...
**For Claude Code:** Generate `CLAUDE.md` using `the Ars Contexta plugin root/generators/claude-md.md` template.
**For Codex:** Generate `CODEX.md` using `the Ars Contexta plugin root/platforms/codex/generator.md` as the platform reference. Codex uses plain-language requests instead of slash-command invocation, so include a command translation table.
...
Step 2: Select feature blocks from the Ars Contexta plugin root/generators/features/.
...
|-----------------|------------|----------|
| `the Ars Contexta plugin root/skill-sources/reduce/` | reduce | Processing |
| `the Ars Contexta plugin root/skill-sources/reflect/` | reflect | Processing |
| `the Ars Contexta plugin root/skill-sources/reweave/` | reweave | Processing |
| `the Ars Contexta plugin root/skill-sources/verify/` | verify | Processing |
| `the Ars Contexta plugin root/skill-sources/validate/` | validate | Processing |
| `the Ars Contexta plugin root/skill-sources/seed/` | seed | Orchestration |
| `the Ars Contexta plugin root/skill-sources/ralph/` | ralph | Orchestration |
| `the Ars Contexta plugin root/skill-sources/pipeline/` | pipeline | Orchestration |
| `the Ars Contexta plugin root/skill-sources/tasks/` | tasks | Orchestration |
| `the Ars Contexta plugin root/skill-sources/stats/` | stats | Navigation |
| `the Ars Contexta plugin root/skill-sources/graph/` | graph | Navigation |
| `the Ars Contexta plugin root/skill-sources/next/` | next | Navigation |
| `the Ars Contexta plugin root/skill-sources/learn/` | learn | Growth |
| `the Ars Contexta plugin root/skill-sources/remember/` | remember | Growth |
| `the Ars Contexta plugin root/skill-sources/rethink/` | rethink | Evolution |
...
the hook shape from `the Ars Contexta plugin root/platforms/codex/hooks/codex-hooks.json.template`.
Use `the Ars Contexta plugin root/platforms/codex/hooks/codex-hooks.sh.template` as
...
Run all 15 primitive checks against the generated system. Use `the Ars Contexta plugin root/reference/validate-kernel.sh` if available. Otherwise manually verify:
...
**If a preset was selected:** Check `the Ars Contexta plugin root/presets/[preset]/starter/` for domain-specific starter notes. Use the most relevant starter as a seed:
