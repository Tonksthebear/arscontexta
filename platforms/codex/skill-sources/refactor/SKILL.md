---
name: refactor
description: Plan vault restructuring from config changes. Compares config.yaml against derivation.md, identifies dimension shifts, shows restructuring plan, executes on approval.
version: "1.0"
...
This is the Codex-native generated workflow template. Invoke it by asking for the workflow in plain language; slash commands in examples are aliases from the Claude version, not requirements.

When instructions mention `$ARGUMENTS`, treat that as the user request target or the relevant file, note, task, topic, or mode from the current prompt. If the target is missing and required, ask one concise clarifying question and wait.
...
Read `the Ars Contexta plugin root/reference/interaction-constraints.md` and check:
...
2. Consult `the Ars Contexta plugin root/reference/` for the latest skill generation approach
